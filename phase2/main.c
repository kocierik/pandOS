...
#define SEMDEVLEN 48 // contando indice 0 = 49
...
pcb_t *queueLowProc; // coda dei processi bassa priorità // VEDI GRUPPO S.O ORAZIO CEO DORMIENTE
pcb_t *queueHighProc; // coda dei processi bassa priorità  // VEDI GRUPPO S.O ORAZIO CEO DORMIENTE
...
void initGlobalVar() {
    activeProc  = 0;
    blockedProc = 0;
    mkEmptyProcQ(&queueLowProc->p_list); //TODO Il codice si blocca entrando in questa funzione: da debuggare. 
    mkEmptyProcQ(&queueHighProc->p_list); 
...
int main(int argc, int* argv[]){
    ...
    insertProcQ(&queueLowProc->p_list, firstProc);
    ++activeProc; 
